...
export interface TaskTitleProps {
  ...
  onUpdateStatus: (value: TagInfo) => Promise<void>;
}
        ...
        taskStatus === 'Not Started' ? (<AtTag name='status' type='primary' circle />) :
          taskStatus === 'Loading' ? (<AtActivityIndicator />) :
            taskStatus === 'Error' ? (<View></View>) :
              taskStatus.status === 'Undone' ? (<AtTag name='status' circle onClick={props.onUpdateStatus} customStyle={{ color: '#78a4f4', borderColor: '#78a4f4', backgroundColor: '#fff' }}>未完成</AtTag>) :
